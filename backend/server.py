                ...
                if os.path.exists(run_sh_file):
                    try:
                        with open(run_sh_file, 'r') as f:
                            ...
                            model_match = run_script.split(
                                "--model ")[1].split(" ")[0] if "--model " in run_script else None
                            if model_match:
                                task_storage[task_id]["model"] = model_match

                            # Check if it's a report with search
                            if "--engine-backend " in run_script:
                                engine_backend = run_script.split(
                                    "--engine-backend ")[1].split(" ")[0]
...
def init_workspace(task_id, model, provider, searcher):
    ...
    with open(env_file, 'w') as f:
        if provider:
            ...
            f.write(
                f"{provider['name']}_BASE_URL={provider['apihost'] if 'apihost' in provider else ""}\n")

        if searcher:
            f.write(f"{searcher['name']}_KEY={searcher['apikey']}\n")
            f.write(
                f"{searcher['name']}_BASE_URL={searcher['apihost'] if 'apihost' in searcher else ""}\n")
...
def run_story_generation(task_id, prompt, model, provider):
    ...
    Init workspace
    """
    records_dir, script_path, output_file, env_file, input_file, done_file, nodes_file = init_workspace(
        task_id, model, provider, None)
...
def run_report_generation(task_id, prompt, model, enable_search, searcher, provider):
    """
    Init workspace
    """

    records_dir, script_path, output_file, env_file, input_file, done_file, nodes_file = init_workspace(
        task_id, prompt, provider, searcher)
    ...
    with open(script_path, 'w') as f:
        ...
        source {env_file}
        export TASK_ENV_FILE={env_file}
        python engine.py --filename {input_file} --output-filename {output_file} --done-flag-file {done_file} --model {model} --searcher {searcher.get("name")} --mode report --nodes-json-file {nodes_file}
        """)
    ...
    task_storage[task_id] = {
        ...
        "start_time": time.time(),
        "model": model,
        "search": searcher if enable_search else None
    }
...
def api_generate_report():
    ...
    thread = threading.Thread(
        ...
        args=(task_id, data['prompt'], data['model'], enable_search,
              data['searchProvider'], data['provider'])
    )
    thread.start()
...
def transform_node_to_graph(node, seen_nodes=None, root=False):
    ...
    if "result" in node:
        ...
        for action_name, action_data in node.get("result", {}).items():
            ...
            action_result = raw_result.get("result", "") if isinstance(
                raw_result, dict) else raw_result
...
def handle_subscribe(data):
    ...
    if not task_id:
        ...
        emit('subscription_status', {'status': 'error',
             'message': 'No taskId provided', 'taskId': None})
        return
